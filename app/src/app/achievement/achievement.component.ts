...
import { Badge } from '../../../../api/src/badge/badge';
...
export class AchievementComponent implements OnInit {
  ...
  displayedColumns: string[] = ['badge', 'assignmentDate', 'achievementDate', 'action'];
  ...
  loadData(): void {
    console.log("here")
    this.service.getAll(this.selectedUser.id as number).subscribe(records => {
      this.records = records
      this.buildForm()
    })
  }
      ...
      rows: this.fb.array(
        this.records.map(
          val => this.fb.group({
            ...
            badge: new FormControl(val.badge),
            assignmentDate: new FormControl(val.assignmentDate),
            achievementDate: new FormControl(val.achievementDate),
            action: new FormControl('existingRecord'),
            isEditing: new FormControl(false),
            isNew: new FormControl(false)
          })
  ...
  update(element: FormGroup){
    ...
    if(record.id){
      this.service.update(this.selectedUser.id as number, record).subscribe(result => {
        this.initialize()
        // this.filterCriteria = ""
      })
    } else {
      this.service.create(this.selectedUser.id as number, record).subscribe(record => {
   ...
   */
  private buildDTO(element: FormGroup): AchievementDto {
    return new AchievementDto(
      element.get('id')?.value as number,
      this.selectedUser.id as number,
      element.get('badge')?.value as number,
      element.get('assignmentDate')?.value as Date,
      element.get('achievementDate')?.value as Date
    )
  }
