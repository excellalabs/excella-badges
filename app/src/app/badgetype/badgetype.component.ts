...
export class BadgeTypeComponent implements OnInit {
  ...
  constructor(
    private service: BadgeTypeService,
   ...
   * @returns 
   */
  isValid(badgetype: BadgeTypeDto){
    if(badgetype.name === '')
      return false
    if(this.records.find(record => (record.name?.toLocaleLowerCase() === badgetype.name.toLocaleLowerCase() && record.id !== badgetype.id)) !== undefined)
      return false 
    return true  
  }
   ...
   */
  private buildDTO(element: FormGroup): BadgeTypeDto {
