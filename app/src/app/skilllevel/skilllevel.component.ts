...
export class SkilllevelComponent implements OnInit {
  ...
  constructor(
    private service: SkilllevelService,
   ...
   * @returns 
   */
  isValid(skilllevel: SkilllevelDto){
    if(skilllevel.name === '')
      return false
    if(this.records.find(record => (record.name?.toLocaleLowerCase() === skilllevel.name.toLocaleLowerCase() && record.id !== skilllevel.id)) !== undefined)
      return false 
    return true  
  }
   ...
   */
  private buildDTO(element: FormGroup): SkilllevelDto {
